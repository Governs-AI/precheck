...
def anonymize_text_presidio(
    ...
    for r in filtered_results:
        ...
        if entity_type in ["API_KEY", "JWT_TOKEN"]:
            ...
            custom_ops[entity_type] = OperatorConfig(
                "replace", {"new_value": entity_type_to_placeholder(entity_type)}
            )
        else:
            # Use mask for PII
            custom_ops[entity_type] = OperatorConfig(
    ...
    out = ANONYMIZER.anonymize(
        text=text, analyzer_results=filtered_results, operators=custom_ops
    ).text
    reasons = sorted(
        {f"pii.redacted:{r.entity_type.lower()}" for r in filtered_results}
    )
    return out, reasons
...
def detect_regex_pii_findings(raw_text: str) -> List[Dict[str, Any]]:
    ...
    _append_regex_findings(
        findings, PHI_MRN, "PII:us_medical_record_number", raw_text, 0.82
    )
    _append_regex_findings(
        findings, PHI_MEMBER_ID, "PII:us_health_member_id", raw_text, 0.8
    )
...
def is_password_field(field_name: str) -> bool:
    ...
    return field_lower in password_fields or any(
        field in field_lower for field in password_fields
    )
...
def redact_obj(
    obj: Any, reasons: Optional[Set[str]] = None, field_name: str = ""
) -> Tuple[Any, Set[str]]:
...
_POLICY_PATH = os.getenv(
    "POLICY_FILE",
    os.path.join(os.path.dirname(__file__), "..", "policy.tool_access.yaml"),
)
...
def apply_tool_access_text(
    tool_name: str, findings: List[Dict], raw_text: str
) -> Tuple[str, List[str]]:
    ...
    for f in sorted(findings, key=lambda x: x["start"], reverse=True):
        ...
        if action == "pass_through":
            ...
            transformed_text = (
                transformed_text[:start] + tokenized_value + transformed_text[end:]
            )
            ...
            transformed_text = (
                transformed_text[:start] + redacted + transformed_text[end:]
            )
...
def apply_tool_access(
    tool_name: str, findings: List[Dict], payload_dict: Dict
) -> Tuple[Dict, List[str]]:
...
def evaluate(
    tool: str, scope: Optional[str], raw_text: str, now: int, direction: str = "ingress"
) -> Dict:
...
def _evaluate_dynamic_policy(
    ...
    try:
        ...
        default_action = defaults.get(direction, {}).get("action", "redact")
        return _apply_default_action_dynamic(
            default_action,
            raw_text,
            now,
            direction,
...
def _apply_tool_specific_policy_dynamic(
    tool: str,
    raw_text: str,
    now: int,
    tool_policy: Dict,
    ...
    if findings:
        ...
        transformed_text, tool_reasons = apply_tool_access_text_dynamic(
            tool, findings, raw_text, allow_pii
        )
        return {
            "decision": "transform",
        ...
        if action == "deny" or action == "block":
            ...
            if user_id and tool_config and policy_config and budget_context:
                budget_result = _check_budget_and_apply(
                    user_id,
                    tool,
                    raw_text,
                    tool_config,
...
def apply_tool_access_text_dynamic(
    tool: str, findings: List[Dict], raw_text: str, allow_pii: Dict[str, str]
) -> Tuple[str, List[str]]:
...
def _apply_default_action_dynamic(
    ...
    network_tools = policy_config.get(
    ...
    if (scope and any(scope.startswith(ns) for ns in network_scopes)) or any(
        tool.startswith(nt) for nt in network_tools
    ):
        if USE_PRESIDIO and ANALYZER is not None:
            redacted_text, reasons = anonymize_text_presidio(raw_text)
        else:
    ...
    # PRECEDENCE LEVEL 5: Strict fallback (only block SSN and passwords)
    return _apply_strict_fallback(
        raw_text, now, user_id, tool_config, policy_config, budget_context
    )


def _apply_strict_fallback(
    raw_text: str,
    now: int,
    user_id: Optional[str] = None,
    tool_config: Optional[Dict] = None,
    ...
    if payment_findings and budget_context and policy_config:
        try:
            ...
            if estimated_purchase is None and tool_config:
                estimated_purchase = get_purchase_amount(
                    tool_config.get("metadata", {})
                )
...
def _check_budget_and_apply(
    ...
    try:
        ...
        budget_status, budget_info = check_budget_with_context(
            budget_context, estimated_llm_cost, estimated_purchase
        )
...
def _add_budget_info_to_result(
    result: Dict,
    user_id: str,
    tool: str,
    raw_text: str,
